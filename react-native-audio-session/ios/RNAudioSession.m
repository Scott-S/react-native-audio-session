...
- (void)routeChange:(NSNotification *)notification
{
    ...
    switch (routeChangeReason) {
        case AVAudioSessionRouteChangeReasonUnknown:
            NSLog(@"routeChangeReason : AVAudioSessionRouteChangeReasonUnknown port");
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonUnknown"}];
            break;

        case AVAudioSessionRouteChangeReasonNewDeviceAvailable:
            // a headset was added or removed
            NSLog(@"routeChangeReason : AVAudioSessionRouteChangeReasonNewDeviceAvailable port");
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonNewDeviceAvailable"}];
            break;

        case AVAudioSessionRouteChangeReasonOldDeviceUnavailable:
            // a headset was added or removed
            NSLog(@"routeChangeReason : AVAudioSessionRouteChangeReasonOldDeviceUnavailable port");
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonOldDeviceUnavailable"}];
            break;

        case AVAudioSessionRouteChangeReasonCategoryChange:
            // called at start - also when other audio wants to play
            NSLog(@"routeChangeReason : AVAudioSessionRouteChangeReasonCategoryChange port");//AVAudioSessionRouteChangeReasonCategoryChange
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonCategoryChange"}];
            break;

        case AVAudioSessionRouteChangeReasonOverride:
            NSLog(@"routeChangeReason : AVAudioSessionRouteChangeReasonOverride port");
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonOverride"}];
            break;

        case AVAudioSessionRouteChangeReasonWakeFromSleep:
            NSLog(@"routeChangeReason : AVAudioSessionRouteChangeReasonWakeFromSleep port");
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonWakeFromSleep"}];
            break;

        case AVAudioSessionRouteChangeReasonNoSuitableRouteForCategory:
            NSLog(@"routeChangeReason : AVAudioSessionRouteChangeReasonNoSuitableRouteForCategory port");
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonNoSuitableRouteForCategory"}];
            break;

        default:
            NSLog(@"routeChangeReason DEFAULT: no change port");
            [self sendEventWithName:@"EventReminder" body:@{@"name": @"AVAudioSessionRouteChangeReasonDEFAULT"}];
            break;
    }
}
...
- (void)interruption:(NSNotification*)notification {
    ...
    switch (interuptionType) {
        case AVAudioSessionInterruptionTypeBegan:
            NSLog(@"Audio Session Interruption case started. port, don't return?");
            break;

        case AVAudioSessionInterruptionTypeEnded:
            NSLog(@"Audio Session Interruption case ended. port, don't return?");
            break;

        default:
            NSLog(@"Audio Session DEFAULT Interruption Notification. port");
            break;
    }
}
...
RCT_EXPORT_METHOD(availableInputs:(RCTResponseSenderBlock)callback)
{
    ...
    BOOL success = [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord withOptions:AVAudioSessionCategoryOptionAllowBluetooth error:nil];
    if (!success) {
        NSLog(@"setCategoryError port");
    }
    ...
    for (int k = 0; k < count; k++) {
        ...
        if ([portDesc.portType isEqualToString:@"BluetoothHFP"]) {
            flag = 0;
            callback(@[portDesc.portName]);
        }
    }
    if (flag) {
...
RCT_EXPORT_METHOD(onAudioChange: (RCTResponseSenderBlock)callback)
{
    callback(@[@"nothing happening here yet."]);
}
